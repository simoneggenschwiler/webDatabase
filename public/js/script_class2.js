...
async function ready() {
    ...
    const res = await fetch(path_json+'/alleles_classII_common.json')
    const allelesCommon = await res.json()
    document.getElementById("nav-toggle").checked = false

    const resFull = await fetch(path_json+'/alleles_classII.json')
...
function createLogoShown(allele_name) {
    ...
    const path_class2 = 'data/classII/'


    for (var spec = 1; spec <= specificity; spec++){
        
        image_name = path_class2 + 'MS/Logos/' + allele.name + '_' + spec + '.png'
